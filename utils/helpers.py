...
readers = {'1': easyocr.Reader(['ch_sim'], model_storage_directory=r'ocr_models'),
           '2': easyocr.Reader(['en'], model_storage_directory=r'ocr_models'),
           '3': easyocr.Reader(['ch_sim', 'en'], model_storage_directory=r'ocr_models')}
...
# ----------比较图片是否相似----------
def are_images_similar(image1, image2, threshold: str):
...
# ----------判断是否是某个对象----------
def is_object(text: str, target: str):
    return text.startswith(f'{target}:') or text.startswith(f'{target}：')


# ----------由条件获取坐标----------
def get_position(content: str, all_data: dict, all_data_index: dict, all_variables: dict):
    parts = content.split('|')
...
# ----------判断范围函数----------
def define_region(content: str, all_variables: dict):
    if content == '':       # 没有给出筛选范围
        return None
...
# ----------识别图片或文字函数----------
def find_image_or_text(contents: list[str], region, all_data: dict, all_data_index: dict, all_variables: dict):
...
# ----------获取图片----------
def get_image(image: str, all_figures: dict, all_variables: dict):
...
# ----------获取数组的内容----------
def get_data_content(text: str, all_data: dict, all_data_index: dict):
...
# ----------获取变量的内容----------
def get_variable_content(text: str, all_variables: dict):
    if not all(char in text for char in ['(', ':', ')']):
...
# ----------把一组由“,”隔开的内容转换成数字元组----------
def get_variable(contents: str, all_variables: dict):
    if ',' not in contents:
...
# ----------判断选择条件是否成立----------
def judge_condition(condition: str, all_data: dict, all_data_index: dict, all_variables: dict, all_figures: dict):
