...
from utils.types import RegionType, PositionType


# 裁剪区域
CROP_REGION = {
    'left': (0, 0.5, 0, 1), 'right': (0.5, 1, 0, 1),
    'up': (0, 1, 0, 0.5), 'down': (0, 1, 0.5, 1),
    'q1': (0, 0.5, 0, 0.5), 'q2': (0.5, 1, 0, 0.5),
    'q3': (0, 0.5, 0.5, 1), 'q4': (0.5, 1, 0.5, 1)
}
...
def find_text(
    ...
    # 确定语言
    if reader is None:
        reader = easyocr.Reader(lang_list=['ch_sim', 'en'], model_storage_directory=r'ocr_models')
    ...
    if region is not None:              # 如果有指定范围
        if isinstance(region, str):     # 范围是一个字符串
            ...
            l, r, u, d = [
                int(ratio * length)
                for ratio, length in zip(crop_para, (width, width, height, height))
            ]
        else:                           # 范围是像素值
